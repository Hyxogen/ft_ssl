...
static const u8 RCON[WHIRLPOOL_NROUNDS + 1][WHIRLPOOL_BLOCK_LEN] = {
    {
...
{
	ctx->offset = 0;
	ft_memset(ctx->state, 0, WHIRLPOOL_BLOCK_LEN);
	mp_init(ctx->nwritten, sizeof(ctx->nwritten));
}

static void add_round_key(u8 block[WHIRLPOOL_BLOCK_LEN], const u8 round_key[WHIRLPOOL_BLOCK_LEN])
{
	for (unsigned i = 0; i < WHIRLPOOL_BLOCK_LEN; i++) {
		block[i] = block[i] ^ round_key[i];
	}
}

static void sub_bytes(u8 block[WHIRLPOOL_BLOCK_LEN])
{
	for (unsigned i = 0; i < WHIRLPOOL_BLOCK_LEN; i++) {
...
static void shift_columns(u8 block[WHIRLPOOL_BLOCK_LEN])
{
	u8 tmp[WHIRLPOOL_BLOCK_LEN];

	ft_memcpy(tmp, block, WHIRLPOOL_BLOCK_LEN);
...
static void mix_rows(u8 block[WHIRLPOOL_BLOCK_LEN])
{
	for (unsigned row = 0; row < 8; row++) {
		mix_row(&block[row * 8]);
	}
...
static void w_transform(u8 block[WHIRLPOOL_BLOCK_LEN], const u8 round_key[WHIRLPOOL_BLOCK_LEN])
{
	sub_bytes(block);
	shift_columns(block);
	mix_rows(block);
...
static void w_cipher(u8 block[WHIRLPOOL_BLOCK_LEN], const u8 key[WHIRLPOOL_BLOCK_LEN])
{
	u8 round_key[WHIRLPOOL_BLOCK_LEN];
...
static void whirlpool_transform(u8 hash[WHIRLPOOL_BLOCK_LEN], const u8 block[WHIRLPOOL_BLOCK_LEN])
{
	/* Miyaguchi-Preneel compression function */

	u8 cipher[WHIRLPOOL_BLOCK_LEN];

	ft_memcpy(cipher, block, WHIRLPOOL_BLOCK_LEN);
...
void whirlpool_update(struct whirlpool_ctx *ctx, const void *buf, size_t n)
{
	...
	for (size_t i = 0; i < n; i++) {
		...
		if (ctx->offset >= WHIRLPOOL_BLOCK_LEN) {
			whirlpool_transform(ctx->state, ctx->block);
			ctx->offset = 0;
		}
	}
...
static const u8 whirlpool_padding[512] = {0x80};
...
static void whirlpool_do_pad(struct whirlpool_ctx *ctx)
{
	...
	offset = (offset + 1) % WHIRLPOOL_BLOCK_LEN;

	size_t stop = WHIRLPOOL_BLOCK_LEN - sizeof(ctx->nwritten);
	if (offset > stop) {
		padding += WHIRLPOOL_BLOCK_LEN - offset;
		padding += stop;
	} else {
		padding += stop - offset;
	}
	...
	whirlpool_update(ctx, whirlpool_padding, padding);
	whirlpool_update(ctx, length, sizeof(length));
}

static void whirlpool_create_hash(unsigned char dest[WHIRLPOOL_DIGEST_LEN],
			    const u8 state[static 512/8])
{
	/* TODO check endiannes */
	ft_memcpy(dest, state, WHIRLPOOL_BLOCK_LEN);
}

void whirlpool_final(unsigned char dest[WHIRLPOOL_DIGEST_LEN], struct whirlpool_ctx *ctx)
{
	whirlpool_do_pad(ctx);
	whirlpool_create_hash(dest, ctx->state);
}
